...
class AuditReport:
    ...
    def to_json(self, indent: int = 2) -> str:
        ...
        -------
        str
            A fully serializable JSON string. Safe to write to disk,
            pipe into CI systems, or POST to a webhook.
            ...
            "breached_columns": [
                {
                    ...
                    "pct_rows_changed": m.pct_rows_changed,
                    "first_leaky_timestamp": str(m.first_leaky_timestamp),
                    "provenance_hint": self.provenance_hints.get(m.column_name, ""),
                }
                for m in self.breached_columns
            ],
...
<body>
  ...
  <footer>
    Generated by <a href="https://github.com/prakulhiremath/temporal-leaks">temporal-leaks</a>
...
def _infer_provenance(col: str, effect_size: float) -> str:
    """
    Heuristic provenance hint. Returns a rule-based description based on the
...
class TemporalAudit:
    ...
    def check(
        ...
        logger.debug(
            "Past rows (≤ T): %d  |  Future rows (> T): %d",
            n_past,
            len(pandas_df) - n_past,
        )
        ...
        # 7. Aggregate leakage score
        leakage_score = float(np.mean([m.effect_size for m in breached])) if breached else 0.0
